    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('ickaxinxi','新增')" @click="jump('../ickaxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="box">
        <ul class="clearfix">
          <li v-for="(item,index) in dataList" v-bind:key="index">
            <a :href="'../ickaxinxi/detail.jsp?id='+item.id">
              <figure>
              </figure>
              <div class="div1">
                <b></b>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#ickahao').val()) {
          param['ickahao'] = jquery('#ickahao').val() ? '%' + jquery('#ickahao').val() + '%' : '';
        }  
        if (jquery('#yonghuhao').val()) {
          param['yonghuhao'] = jquery('#yonghuhao').val() ? '%' + jquery('#yonghuhao').val() + '%' : '';
        }  
        if (jquery('#chepaihaoma').val()) {
          param['chepaihaoma'] = jquery('#chepaihaoma').val() ? '%' + jquery('#chepaihaoma').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('ickaxinxi/list', 'get', param, function(res) {
