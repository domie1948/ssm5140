    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('cheweixinxi','新增')" @click="jump('../cheweixinxi/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="box">
        <ul class="clearfix">
          <li v-for="(item,index) in dataList" v-bind:key="index">
            <a :href="'../cheweixinxi/detail.jsp?id='+item.id">
              <figure>
                <img :src="item.tupian?item.tupian.split(',')[0]:''" width="410" height="272" />
              </figure>
              <h2>{{item.cheweibianhao}}</h2>
              <div class="div1">
                <b></b>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#cheweibianhao').val()) {
          param['cheweibianhao'] = jquery('#cheweibianhao').val() ? '%' + jquery('#cheweibianhao').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('cheweixinxi/list', 'get', param, function(res) {
